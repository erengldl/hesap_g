...
function SidebarLink({ item, collapsed, isActive, badge, staggerIndex }: SidebarLinkProps) {
  ...
  return (
    <Link
      ...
      title={item.name}
      style={{ "--stagger-delay": `${(staggerIndex ?? 0) * 45}ms` } as React.CSSProperties}
      className={cn(
        "group relative flex items-center animate-stagger-item transition-all duration-200",
        collapsed ? "justify-center rounded-xl border px-0 py-3" : "gap-3 rounded-lg px-3 py-2.5",
        isActive
      ...
      <div
        className={cn(
          ...
          isActive
            ...
            : "border-border/70 bg-surface-container/45 text-muted/60 group-hover:border-border-strong group-hover:text-foreground"
        )}
      >
        <item.icon className="h-4 w-4" />
      </div>

      {!collapsed ? <span className="min-w-0 flex-1 truncate text-[0.94rem] font-semibold">{item.name}</span> : null}
      {!collapsed && showBadge ? <NotificationBadge count={badge} /> : null}
      {collapsed && showBadge ? (
...
export default function Sidebar({ collapsed, onToggleCollapse }: SidebarProps) {
  ...
  return (
    <aside className="fixed left-0 top-0 z-50 hidden h-full w-[var(--sidebar-width)] flex-col border-r border-border/80 bg-panel/96 backdrop-blur-2xl transition-[width] duration-300 md:flex">
      <div className="border-b border-border/80 px-3.5 pb-4 pt-4">
        <div className={cn("rounded-2xl border border-border/70 bg-surface-container/58", collapsed ? "px-2 py-3" : "px-3.5 py-3.5")}>
          <div className={cn("flex items-center", collapsed ? "flex-col gap-3" : "gap-3")}>
            <div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-lg border border-primary/30 bg-primary/12 text-primary">
              <LineChart className="h-5 w-5" />
            </div>

            {!collapsed ? (
              <div className="min-w-0 flex-1">
                <p className="truncate text-[0.98rem] font-semibold tracking-[-0.02em] text-foreground">Hesap G</p>
            ...
            <button
              ...
              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-border/80 bg-surface-container/55 text-muted transition-colors duration-200 hover:border-border-strong hover:text-foreground"
            >
      ...
      <nav className="custom-scrollbar flex-1 overflow-y-auto px-3.5 py-4">
        <div className="space-y-5">
          {navigationSections.map((section, sectionIndex) => (
            <section key={section.title}>
              {!collapsed ? (
                <p className="mb-2.5 px-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-muted">{section.title}</p>
              ) : null}
