...
export function classifyDatabaseError(error: unknown): DatabaseErrorInfo {
  ...
  if (rawMessage.includes("DATABASE_URL is required")) {
    return {
      ...
      message: "Production veritabani baglanti degiskeni eksik.",
    };
  }
  ...
  if (code === "28P01" || message.includes("password authentication failed")) {
    return {
      code: "db_auth_failed",
      message: "Veritabani kimlik dogrulamasi basarisiz.",
    };
  }
  ...
  if (code === "ENOTFOUND" || message.includes("getaddrinfo enotfound")) {
    return {
      code: "db_dns_failed",
      message: "Veritabani sunucusu cozumlenemedi.",
    };
  }
  ...
  if (code === "57P01" || code === "57P03") {
    return {
      code: "db_unavailable",
      message: "Veritabani sunucusu su anda erisilebilir degil.",
    };
  }
