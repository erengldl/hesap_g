...
export async function GET(request: Request) {
  ...
  const forecastServiceUrl = process.env.FORECAST_SERVICE_URL?.trim() || null;
  ...
  if (!hasDatabaseUrl()) {
    return NextResponse.json({
      ...
      forecast_service_configured: Boolean(forecastServiceUrl),
  ...
  try {
    ...
    return NextResponse.json({
      ...
      gemini_model: geminiModel,
      forecast_service_configured: Boolean(forecastServiceUrl),
      forecast_service_url: forecastServiceUrl,
      request_id: requestContext?.requestId ?? null,
      timestamp: new Date().toISOString(),
    });
  } catch (error) {
